...
1. write a SQL query to find customers who are either from the city 'New York' or
who do not have a grade greater than 100. Return customer_id, cust_name, city,
grade, and salesman_i
...
Q1) write a SQL query to find customers who are either from the city 'New York' or
who do not have a grade greater than 100. Return customer_id, cust_name, city,
grade, and salesman_i
...
Q2). write a SQL query to find all the customers in ‘New York’ city who have a grade
value above 100. Return customer_id, cust_name, city, grade, and salesman_id.

select * from cust1
where city="New York" and grade>100;

Q3). Write a SQL query that displays order number, purchase amount, and the
...
CREATE table orders (
    ...
    customer_id INT,
    salesman_id INT,
    FOREIGN KEY (customer_id) REFERENCES cust1(customer_id)
);
INSERT INTO orders
VALUES
...
SELECT 
    ord_no,
    purch_amt,
    ROUND((purch_amt / 60) * 100, 2) AS achieved_percentage,
    ROUND(100 - ((purch_amt / 60) * 100), 2) AS unachieved_percentage
FROM 
    orders
WHERE 
    (purch_amt / 60) * 100 > 50;
...
-- Q4. write a SQL query to calculate the total purchase amount of all orders. Return
-- purchase amount.
...
select purch_amt as total_purchase_amount,
customer_id 
from orders
order by total_purchase_amount desc ;

-- Q6. write a SQL query to calculate the average product price. Return average product
-- price.
...
select * from regions;
REPLACE INTO countries VALUES ('IT', 'Italy', 1);
...
-- 12.write a SQL query to find those employees who earn more than the average salary. Return employee ID, 
...
from employees 
where salary<(select employee_id from employees where employee_id=182 );
